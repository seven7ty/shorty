...
const add_link = async (slug, url) => {
  ...
  if (existing) {
    ...
    return new_json_response(
      {
        success: false,
        message: 'Link slug length cannot exceed 30 characters.',
      },
      { status: 400 },
    )
    ...
    return new_json_response(
      {
        success: false,
        message: 'Link slug cannot be one of the reserved words.',
      },
      { status: 400 },
    )
  ...
  return new_json_response(
    { success: true, payload: { slug: slug, url: url } },
    { status: 201 },
  )
}
...
const auth = (request) => {
  if (request.headers.get(AUTH_HEADER) !== AUTH_KEY) {
    return new_json_response(
      { success: false, message: 'Invalid auth key header supplied.' },
      { status: 401 },
    )
  }
}
...
router.put('/:slug', auth, async ({ params, query }) => {
  ...
  if (link) {
    ...
    return new_json_response(
      { success: true, message: 'Link updated.' },
      { status: 200 },
    )
  } else {
    return new_json_response(
      {
        success: false,
        message:
          'Link not found, create it the same way, just with a POST request.',
      },
      { status: 404 },
    )
  }
